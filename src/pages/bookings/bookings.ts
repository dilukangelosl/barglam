...
@Component({
  ...
  providers : [Makeupservice,Bookinservice,Authservice]
})
...
userid:any ;
  ...
  constructor(public navCtrl: NavController, public makeupservice: Makeupservice, public params:NavParams, public bookingservice:Bookinservice, public auth:Authservice, public alertCtrl:AlertController, public  modalCtrl:ModalController, public loadingCtrl:LoadingController) {
...
this.typekey = params.get("typekey");
    ...
    /*
    this.makeupservice.getService(this.typeid,'specialoccasions').$ref.on("value", (res)=>{
      console.log("changed");
      console.log(res);
    })
*/
    ...
    if(this.selected != ""){
  console.log(this.search(this.selected.$key, this.services));
    //  this.service = this.services[this.services.indexOf(this.selected)];
    }
...
changeDate(a:any){
 ...
 // var d = new Date(res);
  ...
  changetime(a:any){
    ...
    var dateComponent = d2.utc().format('YYYY-MM-DD');
    var timeComponent = date.utc().format('HH:mm:ss');


    var res = dateComponent + " "+timeComponent;

    var stringdate = moment(res , 'YYYY-MM-DD HH:mm:ss');

    var d = new Date(stringdate.toString());
...
console.log("time "+ d);
  ...
  ionViewDidLoad() {
    console.log('Hello BookingsPage Page');
  }



  showselect() {
    let contactModal = this.modalCtrl.create(SelectPage, {id:this.typeid, selected:this.selected});
...
search(nameKey:any, myArray:any[]){
  ...
  requestbooking() {
    ...
    if (this.selected.length > 0) {
      ...
      this.event["message"] = this.event["notes"];
      this.event["type"] = "Makeup Artist";
      this.event["dbnode"] = "makeupartist";
      ...
      this.bookingservice.request(this.auth.getAuth().uid, 'makeupartist', this.typeid, this.event).then((res) => {
